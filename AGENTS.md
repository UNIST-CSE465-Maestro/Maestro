...
- `model/`: `PdfDocument` (with `ExtractionStatus` enum: NONE/EXTRACTING/DONE/FAILED, `isPinned`, `bookmarkedPages`), `Folder`, `InkStroke`, `StrokePoint`, `DrawingTool` (PEN/ERASER/LASSO/CROP_CAPTURE), `CropCapturePhase` (IDLE/DRAWING/ADJUSTING), `ChatMessage`, `LlmProvider` (GEMINI/OPENAI/Codex)
...
- **Multi-provider architecture.** Supports Gemini, OpenAI, Codex. Provider selection via `SettingsRepository.getLlmProvider()`/`setLlmProvider()`. Model selection via `getLlmModel()`/`setLlmModel()`. Per-provider API keys stored separately.
- **Provider clients**:
  ...
  - `ClaudeClient`: Anthropic Messages API with SSE streaming. Default model: `Codex-sonnet-4-20250514`.
