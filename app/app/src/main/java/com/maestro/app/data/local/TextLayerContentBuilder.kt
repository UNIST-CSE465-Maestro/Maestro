...
class TextLayerContentBuilder {
    ...
    fun build(documentId: String, index: PdfTextIndex): TextLayerContent {
        val pages = index.pages.map { page ->
            val lines = wordsToLines(page.words)
            val blocks = lines.mapIndexed { blockIndex, line ->
                TextLayerBlock(
                    id = "b$blockIndex",
                    type = "text",
                    bbox = line.bbox,
    ...
    private fun List<PdfTextIndexWord>.bbox(): List<Float> =
        listOf(
            minOf { it.left },
            minOf { it.top },
            maxOf { it.right },
    ...
    private fun PdfTextIndexWord.bbox(): List<Float> =
